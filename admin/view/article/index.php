		  ...
		  <table class="table">
		    ...
		    <?php foreach($article1 as $article): ?>
			    <tr>
			    	...
			    	<td><img width="200px" src="../<?php echo $article['img']; ?>"></td>
			    	<td><a href="?appli=beauty&action=show&id=<?php echo $article['id']; ?>"><?php echo $article['title']; ?></a></td>
			    	<td><?php echo $article['description']; ?></td>
			    	<td><?php echo $article['forh'] ?></td>
			    	<td><?php echo $article['filter'] ?></td>
			    	<td>
			    		<button class="btn"><a href="?appli=article&action=show&id=<?php echo $article['id']; ?>">Show</a></button>
			    	</td>
			    	<td>
			    	...
			    	<td><button class="btn"><a href="?appli=article&action=update&id=<?php echo $article['id']; ?>">Update</a></button></td>
			    	<td>
			    		<form method="post" action="?appli=article&action=desactivate.img" id="formImgDesactivate">
			    			<input type="hidden" id="type" value="<?php echo $article['filter']; ?>">
			    			<input type="hidden" id="id" name="id" value="<?php echo $article['id']; ?>">
			    			<button class="btn btn-default" type="submit">Desactivate</button>
			    		</form>
			    	</td>
			    	<td><a href="?appli=article&action=product&id=<?php echo $article['id']; ?>">Add Product</a></td>
			    	<td></td>
			    </tr>
			<?php endforeach; ?>
		  </table>
	...
	<div id="beaute_inactif">
		<div class="table-responsive">
		  <table class="table">
		    ...
		    <?php foreach($article0 as $article0): ?>
			    <tr>
			    	...
			    	<td><?php echo $article0['description']; ?></td>
